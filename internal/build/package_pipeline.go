...
func buildPackagePipeline(ctx *context, entries []ssaBuildEntry, pkgs []*aPackage, verbose bool) ([]*aPackage, error) {
	buildTasks := make([]*packageBuildTask, len(pkgs))
	for i, pkg := range pkgs {
		buildTasks[i] = newPackageBuildTask(pkg)
	}
	if err := prePackageBuilds(ctx, buildTasks, verbose); err != nil {
		return nil, err
	}
	// Publication is part of each backend task. Initialize the shared cache
	// descriptor on the coordinator before any task can call saveToCache.
	if cacheEnabled() {
	...
	tasks, serialOrder, err := newPackagePipelineTasks(ctx, buildTasks, ssaByPackage)
	if err != nil {
		return nil, err
	}
	...
	for _, task := range tasks {
		task.enabled = !task.build.isRuntime() || hostRuntime
		if task.build.skip {
			task.done = true
			if !task.build.isRuntime() {
				result := packageBuildResultFor(task.build)
	...
	resolveRuntime := func() {
		...
		for _, task := range tasks {
			if !task.build.isRuntime() || task.done {
				continue
			}
	...
	for ssaDone != len(ssaNodes) || !allBackendsDone() || active != 0 {
		for ssaErr == nil && active < parallelism {
			...
			if task := nextBackend(); task != nil {
				...
				go func() {
					...
					observePackagePipeline(ctx, stage, task.build.pkg.PkgPath, true)
					runErr := runPackageJob(task.index, func(int) error {
						...
						_, err := executeAndFinalizePackage(ctx, task.build, verbose, func() error {
							if task.class == pipelineCoordinator {
								return ctx.executeCoordinatorPackageWithCallerTracking(task.build, tracking, verbose)
							}
							return ctx.executeIsolatedPackageWithCallerTracking(task.build, tracking, verbose)
						})
						return err
					})
					observePackagePipeline(ctx, stage, task.build.pkg.PkgPath, false)
		...
		case event.task != nil:
			...
			if event.err == nil && !task.build.isRuntime() {
				result := packageBuildResultFor(task.build)
				needRuntime = needRuntime || result.needRuntime
				needPyInit = needPyInit || result.needPyInit
			}
...
func newPackagePipelineTasks(
	ctx *context,
	buildTasks []*packageBuildTask,
	ssaByPackage map[*ssa.Package]*packagePipelineSSA,
) ([]*packagePipelineTask, []*packagePipelineTask, error) {
	tasks := make([]*packagePipelineTask, len(buildTasks))
	var patchedNormal, coordinatorNormal, patchedRuntime, coordinatorRuntime []*packagePipelineTask
	for i, buildTask := range buildTasks {
		task := &packagePipelineTask{
			index:  i,
			build:  buildTask,
			ownSSA: ssaByPackage[buildTask.pkg.SSA],
		}
		if buildTask.pkg.SSA != nil {
			task.callerRoots = append(task.callerRoots, buildTask.pkg.SSA)
		}
		...
		addCallerNode(buildTask.pkg.SSA)
		...
		for _, dep := range effectiveDependencies(buildTask.pkg) {
		...
		if patch, ok := ctx.patches[buildTask.pkg.PkgPath]; ok {
			addDep(patch.Alt)
			addCallerNode(patch.Alt)
			if patch.Alt != nil && patch.Alt != buildTask.pkg.SSA {
				task.callerRoots = append(task.callerRoots, patch.Alt)
			}
		}

		patched := false
		if _, ok := ctx.patches[buildTask.pkg.PkgPath]; ok {
			patched = true
			task.patched = true
			task.class = pipelineCoordinator
		} else {
			serial, err := ctx.packageRequiresCoordinator(buildTask)
			if err != nil {
				return nil, nil, err
			}
			if serial {
		...
		if task.class == pipelineCoordinator && !buildTask.skip {
			switch {
			case !buildTask.isRuntime() && patched:
				patchedNormal = append(patchedNormal, task)
			case !buildTask.isRuntime():
