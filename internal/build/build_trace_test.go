...
func TestBuildTraceWritesChromeEvents(t *testing.T) {
	...
	ssaSpan.done()
	backendSpan := tracer.startWorker("backend", "example.com/p")
	backendSpan.setArg("class", "isolated")
	tracer.flow(ssaSpan, backendSpan)
	backendSpan.done()
	buildSpan.done()
	...
	var metadata, complete, flowStart, flowEnd bool
	for _, event := range events {
		...
		case "X":
			complete = true
			if event.Name == "backend example.com/p" {
	...
	if !metadata || !complete || !flowStart || !flowEnd {
		t.Fatalf("trace phases: metadata=%v complete=%v flowStart=%v flowEnd=%v", metadata, complete, flowStart, flowEnd)
	}
}
...
func TestBuildTraceRefusesGoSourceOutput(t *testing.T) {
	if _, err := startBuildTrace("trace.go", t.TempDir(), 1); err == nil {
		t.Fatal("startBuildTrace(trace.go) succeeded")
	}
}
