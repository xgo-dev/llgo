...
// PackageSummary is the immutable output a package contributes to the final
// link. It deliberately contains no LLVM values: a backend worker can produce
// it, emit its object, and then dispose its worker-local Program.
//
// C archive/shared header declarations still use LPkg for now. They have a
// separate type-rendering boundary and are kept on the serial compatibility
// path until that boundary is represented here as well.
...
// packageSummaryMetadata is the cache-safe representation of PackageSummary.
// Keep it separate from the in-memory type so the linker-facing structs can
// retain compact unexported record fields.
