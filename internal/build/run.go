...
func runNativeTest(program testProgram, conf *Config, stdout, stderr io.Writer) error {
...
func runNativeTestPrograms(programs []testProgram, conf *Config, stdout, stderr io.Writer) testRunResult {
	...
	return runTestPrograms(programs, parallelism, conf.TestFailFast, conf.TestJSON, stdout, stderr,
		func(program testProgram, output io.Writer) error {
			return runNativeTest(program, conf, output, output)
		})
}
...
func runNative(ctx *context, app, pkgDir, pkgName string, conf *Config, mode Mode) error {
	...
	case ModeTest:
		...
		if err := runNativeTest(program, conf, os.Stdout, os.Stderr); err != nil {
			ctx.testFail = true
		}
	case ModeCmpTest:
