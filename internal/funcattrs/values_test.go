...
func attachValueTestContracts(t *testing.T, mod llvm.Module, name, source string) map[llvm.Value]ValuePlan {
	t.Helper()
	...
	plan, err := PrepareResultAttributes(mod.Context(), mod.NamedFunction(name), sig, attrs, 0, 64, nil)
	if err != nil {
		t.Fatal(err)
	}
	if len(plan) == 0 {
	...
	return map[llvm.Value]ValuePlan{mod.NamedFunction(name): plan}
}
...
`)
	plans := attachValueTestContracts(t, mod, "F", `
//llgo:result(out) nonnull sameas(p)
//llgo:result(n) range(-3,4)
func F(p *int) (out *int, n int8)
`)
	if err := MaterializeValueContracts(mod, plans); err != nil {
		t.Fatal(err)
	}
...
`)
	plans := attachValueTestContracts(t, mod, "Input", `
//llgo:param(p) nonnull
//llgo:param(n) range(-4,5) nonnegative
//llgo:param(element) range(0,10)
...
`)
	plans := attachValueTestContracts(t, mod, "Checked", `
//llgo:result(0) nonnull sameas(p)
