...
func AddCheckFFIFlag(fs *flag.FlagSet) {
	CheckFFI = nil
	fs.BoolFunc("check-libffi", "Automatically select runtime and reflect without libffi when possible (default: false; Go SSA scan, then one backend/LTO)", func(v string) error {
