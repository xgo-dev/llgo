...
func typedmemclr(typ *_type, ptr unsafe.Pointer)             { Typedmemclr(typ, ptr) }
func mapiterinit(t *abi.MapType, m *maps.Map, it *maps.Iter) { mapIterStart(t, m, it) }
func mapiternext(it *maps.Iter)                              { mapIterNext(it) }
func typeString(typ *abi.Type) string                        { return typ.String() }
...
var errNilAssign error = plainError("assignment to entry in nil map")
