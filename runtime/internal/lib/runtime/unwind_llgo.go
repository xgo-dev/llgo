...
//go:linkname c_framepointer2 C.llgo_framepointer
func c_framepointer2() unsafe.Pointer
...
func recoverMark() {
	// Record the helper's own frame address: it sits just below the
	// recovering deferred frame, and the liveness gate tests interval
	// containment, so the exact level does not matter — no chain reads,
	// nothing that can touch a stale slot.
	fp := uintptr(c_framepointer2())
	if fp == 0 {
		return
	}
