...
func TestFunctionAttributeDiagnostics(t *testing.T) {
	for _, tc := range []struct{ source, want string }{
		...
		{"//llgo:cold\nfunc init() {}", "cannot annotate init"},
		{"//llgo:noreturn\nfunc _() {}", "cannot annotate _"},
		...
		_, err = CollectPackageExports(fset, []*ast.File{file})
...
// The dependency has no Go SSA syntax in the caller. Only the imported export
// records can supply its properties; no backend LLVM object is shared.
func TestFunctionAttributesImportedDeclaration(t *testing.T) {
	...
	options := Options{Exports: new(PackageExports)}
	data, err := CollectPackageExports(fset, []*ast.File{depFile})
	if err != nil {
		t.Fatal(err)
	}
	if err := options.Exports.Set(dep, data); err != nil {
		t.Fatal(err)
	}
...
func TestFunctionAttributesPackagePatch(t *testing.T) {
	for _, preloaded := range []bool{false, true} {
		...
		defer prog.Dispose()
		options := Options{Exports: new(PackageExports)}
		data, err := CollectPackageExports(fset, files)
		if err != nil {
			t.Fatal(err)
		}
		if err := options.Exports.Set(patches["p"].Types, data); err != nil {
			t.Fatal(err)
		}
		if preloaded {
...
func G() {}
`)
	index := new(PackageExports)
	data, err := CollectPackageExports(goPkg.Prog.Fset, files)
	if err != nil {
		t.Fatal(err)
	}
	if err := index.Set(goPkg.Pkg, data); err != nil {
		t.Fatal(err)
	}
	ctx := &context{options: Options{Exports: index}}
	f, g := ctx.function(goPkg.Func("F")), ctx.function(goPkg.Func("G"))
...
`)
			...
			source.setAttributes(ctx.functionAttributes(method))
...
func TestPackageExportsCollectAndRoundTrip(t *testing.T) {
	fset := token.NewFileSet()
...
func Remove() { for {} }
`)
	patch := parse("patch.go", "func Remove() {}")
	data, err := CollectPackageExports(fset, []*ast.File{original, patch})
	if err != nil {
		t.Fatal(err)
	}
	...
	if len(data.Functions) != 3 || data.Functions[0].Name != "(*T).Method" || data.Functions[1].Name != "A" || data.Functions[2].Name != "Z" {
		t.Fatalf("unexpected sorted records: %+v", data.Functions)
	}
	encoded, err := yaml.Marshal(data)
	if err != nil {
		t.Fatal(err)
	}
	var restored exportdata.Package
	if err := yaml.Unmarshal(encoded, &restored); err != nil {
		t.Fatal(err)
	}
	again, err := yaml.Marshal(&restored)
	if err != nil {
		t.Fatal(err)
	}
	if string(again) != string(encoded) || !reflect.DeepEqual(data, &restored) {
		t.Fatal("export round trip changed records")
	}
	if got := restored.Function("Remove"); got.Cold || got.NoReturn {
		t.Fatal("replacement retained old attributes")
	}
}
