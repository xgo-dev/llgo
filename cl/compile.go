...
type Options struct {
	...
	// Exports contains validated declaration records associated with loaded packages.
	// Populate it before parallel lowering; nil prepares the current package only.
...
func (p *context) compileInstr(b llssa.Builder, instr ssa.Instruction) {
	...
	case *ssa.Store:
		...
		wasmNilGuard := p.needsWasmNilGuard(va)
		if wasmNilGuard {
			p.recordPanicSite(b, v.Pos())
		...
		if !wasmNilGuard && !isKnownNonNilAddr(va) && !isWrapNilCheckCall(va) {
			p.recordPanicSite(b, v.Pos())
		}
...
func newPackageEx(prog llssa.Program, ct *CallerTracking, patches Patches, rewrites map[string]string, pkg *ssa.Package, files []*ast.File, embedMap *goembed.VarMap, metaCollect bool, options Options) (ret llssa.Package, externs []string, err error) {
	...
	if options.Exports == nil {
		data, exportErr := CollectPackageExports(pkgProg.Fset, files)
		if exportErr != nil {
			return nil, nil, exportErr
		}
		options.Exports = new(PackageExports)
		if exportErr = options.Exports.Set(pkgTypes, data); exportErr != nil {
			return nil, nil, exportErr
		}
	}
