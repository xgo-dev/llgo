...
LLGo currently has two different test-data refresh flows:
...
- Refreshes `expect.txt` for the built-in test suites under `cl/_testlibc`, `cl/_testlibgo`, `cl/_testrt`, `cl/_testgo`, `cl/_testpy`, and `cl/_testdata`.
- Refreshes `meta-expect.txt` under `cl/_testmeta`.
- Preserves the existing skip convention where `expect.txt` containing only `;` means "do not refresh".
- Does not generate LLVM IR checks. All compiler fixtures keep FileCheck directives in their marked Go source.
...
- Generates the same post-target-ABI, pre-LLVM-optimization IR observed by the
  compiler tests. This includes ABI details such as `sret` and indirect
  aggregate arguments.
- Does not update `expect.txt` and does not write separate LLVM golden files.

Every FileCheck run enables `CHECK` plus target prefixes from general to
specific. For example, Darwin/arm64 enables `DARWIN`, `ARM64`, and
`DARWIN-ARM64`; Linux/amd64 enables `LINUX`, `AMD64`, and `LINUX-AMD64`.
Keep portable contracts under `CHECK` and add a target-prefixed directive only
for the line whose ABI or platform spelling genuinely differs. `litgen`
preserves inactive target-prefixed autogenerated regions when one platform is
refreshed.
