...
The full audit requires GNU `timeout`. Each package has a five-minute build/run budget and a separate 60-second guest deadline, extended only for reviewed finite slow packages. LLGo package compilation is cached within the job, but `-count=1` keeps execution mandatory. The normally pattern-excluded `_stress/runtime/timer` package is named explicitly and runs with `LLGO_STRESS_PROFILE=quick`. `other-shard`, `not-run`, and interrupted `incomplete` results are never counted as passes. W3 runs the complete applicable GOROOT corpus on the Go-compatible J32-GoJS path, runs representative GOROOT sentinels on all four LLGo paths, and loads an asynchronous `syscall/js` callback artifact through HTTP in hosted headless Chrome; a missing browser is an error, not a skip.
...
Test binaries have a 60-second test deadline. CI bounds each profile to 25 minutes, or 40 minutes for W32-WASI because every selected package passes through Binaryen/Asyncify. LLGo compilation cache is shared within the profile job, while `-count=1` ensures that every test binary executes. Official Go reference commands force `GOMAXPROCS=1` because the current Go wasm runtimes do not create operating-system threads.
